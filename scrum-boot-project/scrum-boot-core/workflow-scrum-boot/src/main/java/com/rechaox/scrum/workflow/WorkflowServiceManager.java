package com.rechaox.scrum.workflow;
...
//     * @param businessKey 业务key
//     */
//    public void backLastNode(String businessKey) {
...
//        }
//        //  前一个任务
//        HistoricTaskInstance lastTask = hisTaskList.get(0).getId().equals(task.getId()) ?  hisTaskList.get(1) : hisTaskList.get(0);
...
//    }
    ...
    public void rejectTask(String businessKey) {
        Task task = getTaskByBusinessKey(businessKey);
        if (task != null) {
            managementService.executeCommand(new RejectCommand(task.getId(), new HashMap<>(), "驳回"));
        }
    }
